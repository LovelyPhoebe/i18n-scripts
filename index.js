...
function dfs(dir) {
    ...
    children.forEach(s => {
        if (fs.statSync(s).isDirectory() && !excludeDir.some(d => s.includes(d)))
            result.push(...dfs(s));
        else if (['js', 'ts', 'tsx', 'json'].some(u => s.endsWith(u)) && !s.includes('.d.'))
            result.push(s);
    });
    return result;
}
...
function compare(dir, words, i18nDir) {
    ...
    i18nFiles.forEach(fileName => {
        ...
        const translations = JSON.parse(fs.readFileSync(fullPath, { encoding: 'utf-8' }));
        ...
        words.forEach(word => {
            if (translations[word] !== undefined) {
                ...
                if (fileName.indexOf("zh-CN") > -1) {
                    untranslatedEntries[word] = word;
                } else {
                    untranslatedEntries[word] = ""
                }
            }
        });
        const finalTranslations = { ...matchedTranslations, ...untranslatedEntries };

        const outputFileName = path.join(dir, "i18nUtils", fileName.replace('.json', '_filtered.json'));
        
        // Ensure the output directory exists
        ensureDirectoryExistence(outputFileName);
        
        // Write matched translations to a new JSON file
        fs.writeFileSync(outputFileName, JSON.stringify(finalTranslations, null, 2));
    });
}
